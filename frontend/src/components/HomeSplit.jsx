...
export default function HomeSplit() {
  return (
    <div className="relative flex flex-col min-h-screen overflow-hidden md:flex-row">
      ...
      {/* Animated Background */}
      <motion.div
        className="absolute inset-0 z-[-1] bg-gradient-to-r from-sky-100 via-white to-pink-100"
      ...
      <div className="absolute inset-x-0 z-20 px-4 text-center pointer-events-none top-24 md:top-20">
  <motion.h1
    initial={{ opacity: 0, y: 40 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.7 }}
    className="pointer-events-auto text-3xl md:text-6xl font-extrabold 
  ...
  <motion.p
    ...
    className="pointer-events-auto mt-1 text-sm md:text-lg leading-tight 
  ...
  >
    Ignite your career — connect instantly.
      ...
      >
       ...
       <Link to="/recruiter_button" className="w-full max-w-[260px]">
  <motion.button
    whileHover={{ scale: 1.05 }}
    whileTap={{ scale: 0.95 }}
    className="w-full px-6 py-3 font-medium transition bg-white shadow-md md:font-semibold text-sky-600 rounded-xl hover:shadow-lg hover:text-pink-600"
  >
    Post a Job
  </motion.button>
</Link>
      ...
      >
        ...
        <Link to="/jobseeker_button" className="w-full max-w-[260px]">
  <motion.button
    whileHover={{ scale: 1.05 }}
    whileTap={{ scale: 0.95 }}
    className="w-full px-6 py-3 font-medium text-white transition shadow-md md:font-semibold rounded-xl bg-sky-500 hover:bg-sky-600"
  >
    Find Jobs
  </motion.button>
</Link>
